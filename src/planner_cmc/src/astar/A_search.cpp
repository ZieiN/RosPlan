...
void A_search::create_path(geometry_msgs::Point& startPoint, rtp_msgs::PathStamped& pathMsg) {
    Node start(startPoint.x, startPoint.y, 0);
    rtp_msgs::PathPointWithMetadata path_point;
    if(!goal_is_not_reached){
        Node path_node = (*address_of_goal_node).second;
        pathMsg.path_points_with_metadata.clear();
        Node path_node_next;
        path_point.pose.position.x = path_node.getCoordinates().first;
        path_point.pose.position.y = path_node.getCoordinates().second;
        pathMsg.path_points_with_metadata.push_back(path_point);
        bool pathNotComplete = true;


        while(pathNotComplete){
            path_node_next = *(path_node.Pred);
            path_point.pose.position.x = path_node_next.getCoordinates().first;
            path_point.pose.position.y = path_node_next.getCoordinates().second;
            if (path_node_next ==  start){
                pathNotComplete = false;
                pathMsg.path_points_with_metadata.push_back(path_point);
                continue;
